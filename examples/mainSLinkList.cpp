...
int main() {
  ...
  if (found) printf("GetElem(2): %d\n", found->data);  // 应该是 10

  LNode* loc = LocateElem(L, 20);
  if (loc) printf("LocateElem(20): 找到了地址 %p\n", (void*)loc);
  ...
  // 6. 异常删除测试
  if (!ListDelete(L, 0)) printf("6. 非法索引 0 删除拒绝成功\n");
  if (!ListDelete(L, 5)) printf("6. 越界删除拒绝成功\n");

  // 7. 清理内存 (重要)
  // 实际项目中建议写一个 DestroyList 函数，这里简单演示
  while (L->next) ListDelete(L, 1);
  std::free(L);
  printf("\n=== 测试完成，内存已清理 ===\n");
